...
from batch.batch import load_queue, queue_category_from_path  # noqa: E402
from web.server import _run_id  # noqa: E402
from core.tasks import prepare_output_dir  # noqa: E402
...
class TestQueueCategory(unittest.TestCase):
    ...
    def test_prepare_output_dir_uses_category_subdir(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with mock.patch("core.tasks.REPORTS_DIR", Path(tmp)):
    ...
    def test_server_run_id_encodes_category_path(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with mock.patch("web.server.REPORTS_DIR", Path(tmp)):
