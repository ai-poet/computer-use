...
把"分析一款新产品"做成一条可重复流水线:Python 脚本(`backend/scripts/analyze_product.py`)把产品名 + 官网 URL 喂进 `claude --print --output-format stream-json` 子进程,让 Claude Code 走 `product-analyzer` skill 完成抓取/驱动桌面端/截图/写报告,产出 `reports/<slug>-YYYY-MM-DD[-N]/{report.md, metadata.json, screenshots/}`。
...
- `claude_driver.py` — **唯一的子进程编排入口** `run_claude(...)`:spawn → …; cloud batch 可传 `mcp_config` → `claude --mcp-config`。batch 用 `non_interactive` + `log_file`。
...
- `batch.py` — 读取 CSV/JSON 队列,`Semaphore(max_workers)` 限制同时运行的 claude 数,超出部分排队;结束写 `batch-status.json`。
...
- 改任意模块前,先看它在上面 6 子包依赖图的位置,**不要让上游 import 下游**(比如 `core.tasks` 不能 import `analysis.claude_driver`)。
