...
# 批量:每个产品一个本地 Cua sandbox worker
...
- `sandbox_runtime.py` — batch 模式的本地 Cua sandbox 上下文(`sandbox.image`、`local=true`、Android 开关),只生成合约和 env,不替 Claude 直接操作 sandbox。
...
- `batch.py` — 读取 CSV/JSON 队列,用 `asyncio.Semaphore(max_workers)` 控制本地 sandbox worker 并发,每个产品独立 output_dir / metadata / Claude 子进程。
...
`metadata.json` 关键字段:`product_name / url / download_url / host_os / host_arch / runtime ∈ {host, sandbox-local} / sandbox / android / started_at / finished_at / mode ∈ {full, sandbox-full, web-only} / last_session_id / screenshots[] / warnings[]`。
