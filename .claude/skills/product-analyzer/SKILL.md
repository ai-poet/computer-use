...
2. **抓取官网原始信息(线索 + 入门体验)** — 用 `curl -fsSL` 拿 HTML(走父 shell 代理),用 Python 把 `<script>` / `<style>` 剥掉提取纯文本备用,作为后续 grep / 导航的素材。同时通过 cua-driver 打开浏览器做首页全景截图(`01_web_homepage.png`)。如果时间允许,这里就可以顺手把官网各主要页(功能、定价、文档)用 cua-driver 浏览一遍存档 — 但这不能替代后面的桌面端体验
...
5. **驱动目标** —
   ...
   **5.A 完整模式(step 3 命中):桌面应用必做,官网可选深挖** —
   - **桌面端必做**:启动产品,**主要功能挨个走一遍**。每打开一个新界面就 snapshot + screenshot。覆盖到的典型界面:启动页 / 主功能页 / 创建/新建流 / 设置 / 偏好 / 帮助 / 关于 / 错误状态(故意触发一次)。**所有 a11y/click/type/screenshot 必须通过 cua-driver**,直接 `screencapture` / `osascript activate` 是违约。app 段截图至少 4-8 张,**这是 §1 报告的主要素材来源**
   - **官网可选深挖**:在保证桌面端体验完整的前提下,可以用 cua-driver 在浏览器里继续看定价 / 文档 / 博客 / FAQ 等,补充 §3 官网描述的素材。web 段截图数量没有上限 — 但**绝对不能用网页截图替代应用截图**,也不能因为网页内容多就缩水应用体验
