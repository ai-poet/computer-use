...
- **沙箱完整模式 (sandbox-full)**:批量任务专用。用 Cua Sandbox SDK 创建 sandbox(本地 Docker/Lume 或 Cua Cloud),在 sandbox 内完成网页/桌面应用分析;不得操作 host GUI
- **Android 模式 (android)**:增强项。若官网或官方 release 提供 APK,下载 APK 并在 `Sandbox.ephemeral(Image.android(), local=True)` 中安装、启动、截图、体验
...
`runtime` 为 `sandbox-local` 或 `sandbox-cloud` 时还有两条硬约束:

1. **所有操作都发生在 Cua sandbox 内**:用 `from cua import Sandbox, Image` 创建 ephemeral sandbox(`local=True` 本地 / `local=False` 云端),通过 `sb.shell.run()` / `sb.screenshot()` / `sb.mouse` / `sb.keyboard` 操作。不得启动或驱动 host 浏览器/host 桌面应用,也不得依赖 host cua-driver
2. **每个产品一个独立 sandbox 生命周期**:批量任务里的 Claude worker 只负责当前产品;进入任务后创建 sandbox,结束任务前让 context manager 清理。不要复用其它产品的 sandbox 或输出目录

3. **批量并行(`ANALYZER_BATCH_PARALLEL=1`)时沙盒独占**:prompt 会明确要求**全部**操作系统级工作在沙盒内完成。即使 host 上有 curl/浏览器/代理,也不得在 host 父 shell 抓官网或点 GUI;`curl`、开浏览器、下载安装包、点击、截图一律在 `sb.shell` / `sb.mouse` / `sb.keyboard` / `sb.screenshot` 内完成。host 仅读写 `output_dir`。

Linux 桌面优先 `Image.linux(kind="container")` + registry **`trycua/cua-xfce:latest`**,`DockerRuntime(platform="linux/amd64")`;不要用默认 `kind=vm` 的 QEMU 路径,除非 `sandbox.image` 明确要求完整 VM。环境变量 `ANALYZER_LINUX_CONTAINER_IMAGE` / `ANALYZER_LINUX_DOCKER_PLATFORM` 由 orchestrator 注入。

**Host 侧 Python(沙盒模式必读)**:Cua Sandbox SDK 要求 **Python 3.12 或 3.13**。编排器环境变量 `ANALYZER_PYTHON_VERSION_MIN`、`ANALYZER_CONDA_ENV`(推荐 `computer-use-py312`) 已注入。在 host 上写/执行 `import cua` 的脚本前,应 `conda activate` 对应环境并确认 `python --version`;勿用 macOS 自带 3.9。沙盒内 `sb.shell.run("python3 ...")` 用容器 Python,与 host SDK 版本无关。

### 7 步详细

1. **TodoWrite 建 7 个 todos**:`抓取官网` → `定位下载链接` → `安装/启动应用` → `驱动目标(应用 + 可选网页)` → `批量截图` → `整合报告` → `补齐 metadata.json`。每步开始前 `in_progress`,完成转 `completed`

2. **抓取官网原始信息(线索 + 入门体验)** — 用 `curl -fsSL` 拿 HTML(`sandbox-local` / `sandbox-cloud` 或 `ANALYZER_BATCH_PARALLEL=1` 时**必须在 sandbox `sb.shell` 内**跑,禁止 host 父 shell 捷径;仅 host 单任务可走父 shell 代理)。用 Python 剥 `<script>` / `<style>` 提纯文本。同时在沙盒内打开浏览器做首页截图(`01_web_homepage.png`)。可顺带浏览定价/文档页 — 不能替代桌面端体验
...
sandbox-local / sandbox-cloud runtime 下,遵守调用方 prompt 给出的 Cua Sandbox SDK 合约:每个产品一个 sandbox,用 `Sandbox.ephemeral(Image..., local=<sandbox.local>)` 创建,用 `sb.shell` / `sb.screenshot` / `sb.mouse` / `sb.keyboard` 操作,不得操作 host GUI。
