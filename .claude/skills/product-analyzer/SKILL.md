...
| 3 | `step scroll` | 滚轮翻页 |
| 4 | `step type` / `step key` | 输入文字、`enter`/`ctrl+l` 等小写键名(勿用 `Return`/`Escape`) |
| 5 | `step open-url` / `bootstrap --url` | 打开官网(键盘进地址栏,非 wget) |
| 末位 | `step shell -c '...'` | **仅**见下方例外 |

**`step shell` 只允许这些场景**(其余一律鼠标流):

1. **bootstrap 时启动 Chromium 一次**(脚本内部已做;你不要手写长 shell 链)
...
python scripts/sandbox_ctl.py step click "$OUTPUT_DIR" 640 120    # 示例:关 cookie
