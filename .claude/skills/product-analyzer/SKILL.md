...
- `sandbox.local` — `true` 为本地 Docker/Lume/QEMU;`false` 为 Cua Cloud(需 `CUA_API_KEY`,由 orchestrator 注入 env,**不要**写入 metadata.json)
...
Linux 桌面优先 `Image.linux(kind="container")`(Docker/XFCE),不要用默认 `kind=vm` 的 QEMU 路径,除非 `sandbox.image` 明确要求完整 VM。
