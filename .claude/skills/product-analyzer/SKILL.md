...
- **沙箱完整模式 (sandbox-full)**:批量任务专用。用 Cua Sandbox SDK 创建本地 sandbox,在 sandbox 内完成网页/桌面应用分析;不得操作 host GUI
...
- `runtime` — `host` 或 `sandbox-local`。`host` 使用当前主机 + cua-driver;`sandbox-local` 必须使用 Cua Sandbox SDK 创建本地 sandbox
- `sandbox.image` — `auto` / `linux` / `macos` / `windows`,仅 `sandbox-local` 有效
...
`runtime=sandbox-local` 时还有两条硬约束:

1. **所有操作都发生在 Cua sandbox 内**:用 `from cua import Sandbox, Image` 创建本地 ephemeral sandbox,通过 `sb.shell.run()` / `sb.screenshot()` / `sb.mouse` / `sb.keyboard` 操作。不得启动或驱动 host 浏览器/host 桌面应用,也不得依赖 host cua-driver
...
2. **抓取官网原始信息(线索 + 入门体验)** — 用 `curl -fsSL` 拿 HTML(走父 shell 代理;sandbox-local 时在 sandbox shell 内跑),用 Python 把 `<script>` / `<style>` 剥掉提取纯文本备用,作为后续 grep / 导航的素材。同时打开浏览器做首页全景截图(`01_web_homepage.png`)。如果时间允许,这里就可以顺手把官网各主要页(功能、定价、文档)浏览一遍存档 — 但这不能替代后面的桌面端体验
...
4. **安装/启动应用(仅当 step 3 命中)** —
   ...
   - sandbox-local runtime:按 `sandbox.image` 或安装包平台选择 `Image.macos()` / `Image.windows()` / `Image.linux()`,用 `Sandbox.ephemeral(..., local=True)` 创建 sandbox,在 `sb.shell.run()` 内下载/安装/启动。所有截图用 `await sb.screenshot()`
   - Android APK:若 `metadata.android.apk_url` 已填,额外创建 `Sandbox.ephemeral(Image.android(), local=True)`,在 Android sandbox 内下载或传入 APK,用 `adb install` / sandbox shell 安装,通过 `cmd package resolve-activity` / `monkey` / 启动 intent 获取并启动包名,写入 `metadata.android.package_name` 与 `metadata.android.mode`
...
sandbox-local runtime 下,遵守调用方 prompt 给出的 Cua Sandbox SDK 合约:每个产品一个本地 sandbox,用 `Sandbox.ephemeral(Image..., local=True)` 创建,用 `sb.shell` / `sb.screenshot` / `sb.mouse` / `sb.keyboard` 操作,不得操作 host GUI。
