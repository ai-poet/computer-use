...
Linux 桌面优先 `Image.linux(kind="container")` + registry **`trycua/cua-ubuntu:latest`**,`DockerRuntime(platform="linux/amd64")`;不要用默认 `kind=vm` 的 QEMU 路径,除非 `sandbox.image` 明确要求完整 VM。环境变量 `ANALYZER_LINUX_CONTAINER_IMAGE` / `ANALYZER_LINUX_DOCKER_PLATFORM` 由 orchestrator 注入。
