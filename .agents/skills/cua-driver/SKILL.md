---
name: cua-driver
description: Drive a native desktop app via the cua-driver CLI (default) or MCP server — snapshot its accessibility tree, click/type/scroll by element_index, verify via re-snapshot. Backend is auto-selected per platform: macOS uses the Swift `cua-driver` (mature), Linux/Windows use the Rust port `cua-driver-rs`. The CLI/MCP surface is identical across backends. Use when the user asks you to operate, drive, automate, or perform a GUI task in a real desktop application on the host (e.g. "open a file in TextEdit", "click the Save button in Numbers", "fill the form in Notepad").
---

# cua-driver

Orchestrates desktop app automation via `cua-driver`. Whenever a user
asks to drive a native app, follow the loop in this skill rather
than calling tools ad-hoc — the snapshot-before-action invariant is not
optional and silently breaks if you skip it.

## Backend selection

| Host OS | Binary | Notes |
|---|---|---|
| macOS (arm64 / x86_64) | Swift `cua-driver` | mature, official, in `/Applications/CuaDriver.app` |
| Linux x86_64 | `cua-driver-rs` (Rust port) | feature-complete on Linux |
| Windows x86_64 | `cua-driver-rs` (Rust port) | installs via `install.ps1`, no admin required |
| Linux aarch64 | not supported by prebuilts | source build only |

The CLI subcommands and MCP tools described in this skill are the
...
`backend/scripts/install_cua_driver.py` from this project.
...
For normal Codex use, keep the default CLI or `cua-driver` MCP server path above. If the user explicitly wants Codex's vision/computer-use-style flow, they can register:

```bash
Codex mcp add --transport stdio cua-computer-use -- cua-driver mcp --Codex-computer-use-compat
```

Observation: Codex vision flows appear to treat a screenshot MCP tool as the image-grounding anchor. This compatibility mode keeps the normal CuaDriver tools and changes only `screenshot`. The compatibility `screenshot` requires `pid` and `window_id`, captures only that target window, and returns the window-local pixel coordinate frame. Start with `launch_app` or `list_windows`, then call `screenshot({pid, window_id})`; do not assume desktop coordinates or a full-screen capture.

Use MCP for this Codex vision/computer-use-style path. Do not shell out to `cua-driver screenshot` as a substitute: CLI screenshots still work as CuaDriver calls, but they do not expose the `mcp__cua-computer-use__screenshot` tool name that Codex appears to use as the image-grounding cue.
...
3. Start the daemon with `open -n -g -a CuaDriver --args serve` (the
   ...
   detects a wrong-TCC context (any IDE-spawned shell: Codex,
...
Pure-vision mode has its own caveats — Codex's vision
