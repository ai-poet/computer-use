...
**网页点击能不能用?** 可以。`cua-xfce` 镜像是带 XFCE 桌面的 Linux 容器,内置 `computer-server` 与 **Firefox**;用 `sandbox_ctl bootstrap --open-browser --url …` 或 `step open-url` 打开官网,再用 `step click/type/screenshot` 做**坐标级**自动化(勿假设有 Chromium)。这与 host 上 cua-driver 的 `element_index` 不是同一条路,但对常规官网导航、表单、滚动足够。验证脚本:`python -m tests.sandbox.sandbox_ctl_smoke`(走 bootstrap + Firefox + scroll/click,截图在 [`tmp/sandbox-ctl-smoke/screenshots/`](tmp/sandbox-ctl-smoke/screenshots/)的 `01_web_*.png` … `04_web_*.png`)。
...
# 多个队列按优先级顺序跑(重复 --batch,顺序由命令行决定;TTY 下共用一个 curses 控制台)
python3 scripts/analyze_product.py \
...
`--batch` 与 `--batch-all` 二选一,不能同时使用。`--batch` 可重复指定,按命令行顺序合并为一条队列(FIFO 调度);TTY 下共用一个 curses 批量控制台。加 `--batch-plain` 才会禁用控制台。
