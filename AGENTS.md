...
- `renderer.py` — `format_event(line, state)`:把 stream-json 单行 JSON 翻译成 Codex 风格的彩色终端输出(thinking / tool_use / tool_result / TodoWrite 列表 / result)。**不直接 print,把行返回给上层**,让 Spinner 控制擦除/重画时机。
- `preflight.py` — `detect_host()` 归一化 OS+arch;`ensure_claude_cli` 检查 `which Codex`;`ensure_cua_driver` 缺则自动调 `install_cua_driver.py`;`ensure_cua_sdk` / `check_local_sandbox_prereqs` 支撑本地 sandbox batch。
...
- `sandbox_runtime.py` — batch 模式的 Cua sandbox 上下文(本地/云端、`sandbox.image`、`CUA_API_KEY`/`NO_PROXY` env)、`write_cloud_mcp_config`。
- `sandbox_ctl.py` — 本地 batch 逐步控制桥(`bootstrap`/`step`/`teardown`/`status`),写 `<output_dir>/sandbox.json`;不 import `claude_driver`。
- `prompts.py` — `build_prompt`(传 runtime/sandbox 参数并指向 skill「沙盒控制三阶段」)和 `build_resume_prompt`(从 session 续跑)。沙盒操作细则在 skill,不在 prompt 重复。
- `claude_driver.py` — **唯一的子进程编排入口** `run_claude(...)`:spawn → …; cloud batch 可传 `mcp_config` → `Codex --mcp-config`。batch 用 `non_interactive` + `log_file`。
- `batch_store.py` — `BatchJob` 状态机(排队/运行/暂停/完成)与线程安全快照,供控制台与 worker 共享。
- `batch_dashboard.py` — TTY 下 curses 双层 UI:列表总览(并发/排队/进度条)与 Enter 详情;详情 Esc 可暂停单任务并 `--resume` 续跑(暂停仍占并发槽)。
- `batch.py` — 读取 CSV/JSON 队列,`Semaphore(max_workers)` 限制同时运行的 Codex 数,超出部分排队;结束写 `batch-status.json`。
...
```
Codex --print --output-format stream-json --verbose
...
- stdout 是逐行 stream-json,`renderer.format_event` 解析。`system.subtype=init` 事件里的 `session_id` 抓住后**立刻**通过 `update_metadata(out_dir, last_session_id=...)` 写盘,这样即使 Codex 中途被 kill 也能 `--resume`。
...
- `stale` — 连 `last_session_id` 都没有(Codex 没真正起跑过)— 不能恢复,提示删目录重跑

### Skill 调用链

`build_prompt` 的内容**显式**告诉 Codex "请使用 product-analyzer skill 完成产品分析",并把 `output_dir`、主机信息、产品名/URL 列出来。Codex 读 `.Codex/skills/product-analyzer/SKILL.md` 进入 7 步 canonical loop,在 step 3-5 通过 `cua-driver` skill 操作浏览器/桌面应用。两个 skill 的分工:**product-analyzer 决定"分析什么、产出什么";cua-driver 决定"如何不抢焦点地驱动桌面"**。
...
任何驱动桌面端的工作都受 `.Codex/skills/cua-driver/SKILL.md` 约束。最重要一条:**用户的前台应用不能变**。这意味着以下命令在本仓库里**绝对禁用**(除非用户**明确**要求"切到前台"):
...
正确做法:**`launch_app({bundle_id, urls:[...]})` 起进程并打开 URL**(内部有 FocusRestoreGuard),后续操作走 `get_window_state` → `element_index` 点击 → 再 `get_window_state` 校验。详尽规则在 `.Codex/skills/cua-driver/SKILL.md`。
...
├── metadata.json        # 机器可读元数据,Python 写雏形,Codex 在结束前补齐
